...
*   **Template Variables**: Within a shortcode template file OR a callable shortcode, the following variables are available:
    *   `$params`: An associative array of the attributes passed to the shortcode. This array now supports both named attributes (accessed by their key, e.g., `$params['name']`) and positional attributes (accessed by numeric keys, e.g., `$params[0]`). All positional attributes are also available as an array under the special `'_'` key (e.g., `$params['_']`).
